	...
	haruka.on('group-participants-update', async (anu) => {
		...
		try {
						...
						if (anu.action == 'add' && !num.includes(haruka.user.jid)) {
							...
							try{
							await sendButLoc(mdata.id, `Welcome @${num.split('@')[0]} to ${mdata.subject}` + '\n' + lang.welcome(), `Klik untuk daftar`,welcome, [{"buttonId": `.shop`,"buttonText": {"displayText": "Shop"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							} catch {
								await sendButLoc(mdata.id, `Welcome @${num.split('@')[0]} to ${mdata.subject}` + '\n' + lang.welcome(), `Klik untuk daftar`,ppmem2, [{"buttonId": `.shop`,"buttonText": {"displayText": "Shoo"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							}
